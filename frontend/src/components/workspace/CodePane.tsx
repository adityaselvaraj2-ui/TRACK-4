...
export function CodePane({
  ...
  const handleSave = async () => {
    if (!active || draft === undefined || readOnly) return;
    setSaving(true);
    try {
              ...
              files.map((f) => (
                <button
                  ...
                  className={cn(
                    ...
                    activePath === f.path ? "bg-gradient-brand text-primary-foreground panel-active" : "text-muted-foreground hover:text-foreground",
                  )}
                >
            ...
            {!readOnly && (
              <button
                ...
                className={cn(
                  ...
                  dirty ? "bg-gradient-brand text-primary-foreground neon-border" : "text-muted-foreground/50",
                )}
              >
